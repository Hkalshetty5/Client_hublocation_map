...
if uploaded_file is not None:
    ...
    # Clean whitespace from Hub Name and Name columns
    ...
    for _, client_row in client_data.iterrows():
        ...
        if not hub_row.empty:
            hub_row = hub_row.iloc[0]  # Get the first matching hub
            folium.PolyLine(
                locations=[
                    [client_row['LATITUDE'], client_row['LONGITUDE']],  # Client location
                    [hub_row['Lat'], hub_row['Long']]  # Hub location
                ],
                color=hub_colors.get(hub_name, 'black'),  # Use the assigned color for this hub
                weight=3,  # Line thickness
                opacity=0.6  # Line opacity
            ).add_to(mymap)
        else:
            st.write(f"Warning: Hub '{hub_name}' for client '{client_row['CLIENT WAREHOUSE CODE']}' not found in HubSheet.")
