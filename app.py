...
if uploaded_file is not None:
    ...
    # Standardize Hub Name columns to avoid mismatches
    client_data['Hub Name'] = client_data['Hub Name'].str.strip().str.upper()
    hub_data['Name'] = hub_data['Name'].str.strip().str.upper()
    ...
    # Draw lines from each client to their hub
    for _, client_row in client_data.iterrows():
        hub_name = client_row['Hub Name']
        if pd.notna(hub_name):
            matching_hub = hub_data[hub_data['Name'] == hub_name]
            if not matching_hub.empty:
                hub_row = matching_hub.iloc[0]
                folium.PolyLine(
                    locations=[
                ...
                st.write(f"Warning: Hub '{hub_name}' for client '{client_row['CLIENT WAREHOUSE CODE']}' not found.")
    ...
    st.download_button(
        ...
        data=map_data.getvalue(),
        file_name="client_hub_map.html"
    )
else:
